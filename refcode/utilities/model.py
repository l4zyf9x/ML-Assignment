...
class Model:
    ...
    def train(self, data, labels, batch_size=256, epochs=50, optimization='adam',
              save_model=True, load_and_continue=False, display_after=20):
        ...
        for epoch in range(epochs):
            ...
            for i, (x_batch, y_batch) in enumerate(get_batches(data, labels)):
                ...
                for layer in self.model:
                    if layer.has_weights():
                        layer.apply_grads(
                            optimization=optimization, correct_bias=True, iter=iter)
            for layer in self.model:
                if layer.has_weights():
                    layer.save_weights(path.join(get_models_path(), self.name))
                loop += 1
                if loop % display_after == 0:
                    loss = self.loss.compute_loss(y_batch, batch_preds)
                    acc = evaluate(y_batch, batch_preds)
                    print('Epoch: {}, batch: {}, loss:{}, acc'.format( epoch + 1, i, loss))
            iter += batch_size
